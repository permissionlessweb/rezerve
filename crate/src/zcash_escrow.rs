...
//! not spend authority). Spend is DKG FROST (`frost-spend-v1` over the opened
//! Ironwood nullifier, copied into the dest memo) then `add_ironwood_spend` of
//! **that** opened note, proven by its nullifier. Close is two such spends of
//! that note's lineage (earned + remainder). Prefer a shielded-only DKG
//! Ironwood spend (ZIP-317 from the note). Miner coinbase pays ZIP-317 only
//! when the note cannot cover the fee — never a wallet sidecar send (send-many
//! / shield-coinbase RPCs) and never a fresh coinbase re-shield as if
//! FROST-owned. Not Sapling/Orchard. Not a RAM u64 pool as money.
//!
//! Spend authority is the DKG group. Any valid t-of-n subset reconstructs the
//! same FROST-ed25519 *seed*, then ZIP-32 derives the Ironwood `SpendingKey`
//! (coin type 133). The public verifying key is not sufficient and the
//! reconstructed ed25519 scalar is not stuffed into `SpendAuthorizingKey`
//! (wrong curve). FROST-ed25519 `sign_with_seats` over the Ironwood nullifier
//! is the product spend grant; RedPallas FROST (ZIP-312) is not this module.
//!
//! Honest production stand-in: this Zakura / orchard crate does not implement
//! ZIP-312 RedPallas FROST-owned Ironwood spend. Do not fake it. Keep
//! `frost-spend-v1` + ZIP-32 of the reconstructed DKG group seed. `PIR_ZAKURA_LAB=1`
//! still fails closed without a live node.
...
/// Honest production stand-in while ZIP-312 RedPallas FROST is not in this
/// Zakura: DKG seats reconstruct the frost-ed25519 group seed, ZIP-32 (coin
/// type 133) derives the Ironwood `SpendingKey`, and `frost-spend-v1` over the
/// opened note's nullifier is the spend grant (dest memo). Not miner coinbase
/// as FROST-owned. Not a fake ZIP-312.
pub const IRONWOOD_SPEND_STAND_IN: &str =
...
/// Nullifier of an opened DKG Ironwood note. Close's first `add_ironwood_spend`
/// must consume **this** note; the second spends the remainder of the same lineage.
/// Not miner coinbase. Not ZIP-312 (see [`IRONWOOD_SPEND_STAND_IN`]).
pub fn dkg_opened_nullifier(
    seats: &[FrostSeat],
    note: &ShieldedNote,
    ...
    require_dkg_owns_opened(&fvk, &opened)?;
    Ok(opened.nullifier(&fvk).to_bytes())
}

/// Shared receiver: orchard address bytes + incoming viewing key (IVK).
/// Depositors send to `ua`; the wallet service identifies notes with `ivk`.
pub fn dkg_shared_ua_ivk(seats: &[FrostSeat], tag: &[u8]) -> Result<(String, String), ZcashEscrowError> {
    let sk = ironwood_sk_from_seats(seats)?;
    let fvk = FullViewingKey::from(&sk);
    let addr = fvk.address_at(dest_diversifier(tag), Scope::External);
    let ivk = fvk.to_ivk(Scope::External);
    Ok((addr_hex(addr), hex::encode(ivk.to_bytes())))
}

impl ZcashEscrow {
    /// Scan recent blocks for Ironwood outputs decryptable by this group's IVK.
    pub fn scan_incoming(
        &self,
        seats: &[FrostSeat],
        tag: &[u8],
        from_height: u64,
    ) -> Result<Vec<(String, u64)>, ZcashEscrowError> {
        let sk = ironwood_sk_from_seats(seats)?;
        let fvk = FullViewingKey::from(&sk);
        let want = fvk.address_at(dest_diversifier(tag), Scope::External);
        let tip = self
            .rpc_result("getblockcount", Value::Array(vec![]))?
            .as_u64()
            .unwrap_or(0);
        let mut found = Vec::new();
        let start = from_height.min(tip);
        for h in start..=tip {
            let hash = match self
            ...
            let block = self.rpc_result("getblock", serde_json::json!([hash, 2]))?;
            ...
            for txv in txs {
                ...
                let txid = txv
                    ...
                    .unwrap_or("")
                    .to_string();
                let parsed = if let Some(hx) = hex_tx {
                    parse_v6_tx(hx).ok()
                } else if !txid.is_empty() {
                    self.rpc_result("getrawtransaction", serde_json::json!([txid, 0]))
                        .ok()
                        .and_then(|v| v.as_str().map(|s| s.to_string()))
                        .and_then(|hx| parse_v6_tx(&hx).ok())
                } else {
                    None
                };
                let Some(tx) = parsed else { continue };
                if let Some(n) = decrypt_output_to(tx.ironwood_bundle(), &fvk, want) {
                    found.push((txid.clone(), n.value().inner()));
                }
            }
        }
        Ok(found)
...
pub struct SpendResult {
    ...
    pub nullifier: [u8; 32],
    /// ZIP-317 paid from the opened DKG Ironwood note (no miner transparent).
    pub shielded_only: bool,
    /// Honest production stand-in for this spend (not ZIP-312). See
    /// [`IRONWOOD_SPEND_STAND_IN`].
    pub spend_stand_in: &'static str,
}

impl ShieldedNote {
    /// Spend authority for this opened DKG Ironwood note: frost-spend-v1 +
    /// ZIP-32 of the reconstructed group seed. Not ZIP-312 RedPallas FROST.
    pub fn spend_stand_in(&self) -> &'static str {
        let _opened = (!self.ironwood_note.is_empty(), self.group_id, self.amount_zat);
        let _ = _opened;
        IRONWOOD_SPEND_STAND_IN
    }
}
...
impl ZcashEscrow {
    ...
    /// Advance the lab tip so close height can exceed open height for `f`.
    pub fn generate(&self, n: u64) -> Result<Vec<String>, ZcashEscrowError> {
        self.generate_blocks(n)
    }

    fn send_raw(&self, hex_tx: &str) -> Result<String, ZcashEscrowError> {
        let params = serde_json::json!([hex_tx]);
        let v = match self.rpc_result("submitrawtransaction", params.clone()) {
            Ok(v) => v,
            Err(_) => self.rpc_result("sendrawtransaction", params)?,
        };
        v.as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| ZcashEscrowError::NoteOp(format!("submitrawtransaction: {v}")))
    }
    ...
    /// so spend can `add_ironwood_spend` it. Any valid threshold subset of the
    /// same DKG run owns the ZIP-32 Ironwood SK (group seed, not a seat-pair
    /// hash and not the public VK).
    /// Spend authority is [`IRONWOOD_SPEND_STAND_IN`]: frost-spend-v1 grant +
    /// ZIP-32 of the reconstructed DKG group seed. Not ZIP-312 RedPallas FROST.
    pub fn open_note(
        &self,
        group: &FrostGroup,
        seats: &[FrostSeat],
        ...
        self.wait_tx(&txid)?;
        let note = confirm_mined_dkg_open(self, &txid, seats, &note)?;
        let tip = self
            .regtest()
            .commitment_tree_root()?
            .unwrap_or_else(|| txid.clone());
        let commitment = extracted_cmx(&note);
        let opened = ShieldedNote {
        ...
        // Threshold DKG seats own this note. Close is two add_ironwood_spend of
        // THAT note under IRONWOOD_SPEND_STAND_IN (not ZIP-312).
        if opened.spend_stand_in() != IRONWOOD_SPEND_STAND_IN {
            return Err(ZcashEscrowError::NoteOp(
                "DKG open: ZIP-312 RedPallas FROST is not this module (keep frost-spend-v1 + ZIP-32 reconstructed seed)".into(),
            ));
        }
        Ok(opened)
    }
    ...
    pub fn spend_note(
        ...
        if note.spend_stand_in() != IRONWOOD_SPEND_STAND_IN {
            return Err(ZcashEscrowError::NoteOp(
                "DKG spend: ZIP-312 RedPallas FROST is not this module (keep frost-spend-v1 + ZIP-32 reconstructed seed)".into(),
            ));
        }
        let opened = decode_ironwood_note(&note.ironwood_note)?;
        ...
        let fvk = FullViewingKey::from(&sk);
        require_dkg_owns_opened(&fvk, &opened)?;
        let nf = opened.nullifier(&fvk).to_bytes();
        // Product spend grant is FROST over this Ironwood nullifier (DKG group),
        // not wallet send-many of miner transparent.
        let msg = crate::tagged_hash(
            b"frost-spend-v1",
            &[
                &note.group_id,
                &note.commitment,
        ...
        let (txid, change_note, nullifier, shielded_only) =
            spend_opened_ironwood(self, seats, &opened, dest_addr, amount_zat, &frost_sig)?;
        self.generate_blocks(1)?;
        self.wait_tx(&txid)?;
        let mined = confirm_mined_dkg_spend(
            self,
            &txid,
            seats,
            &opened,
        ...
        let mined_change = decrypt_output_to(mined.ironwood_bundle(), &fvk, change_addr);
        if change_note.is_some() && mined_change.is_none() {
            return Err(ZcashEscrowError::NoteOp(
                "DKG spend: mined tx missing DKG change note (cannot re-shield coinbase)".into(),
            ));
        }
        let change = mined_change.or(change_note).map(|n| {
        ...
        Ok(SpendResult {
            ...
            nullifier,
            shielded_only,
            spend_stand_in: IRONWOOD_SPEND_STAND_IN,
        })
    }

    /// Close: two `add_ironwood_spend` of **that** opened DKG Ironwood note.
    /// First consumes the opened note (earned → provider + DKG change). Second
    /// consumes the remainder note (tenant). Not two spends of miner coinbase,
    /// not a cloned provider tx as a fake refund, not ZIP-312 (see
    /// [`IRONWOOD_SPEND_STAND_IN`]).
    pub fn close_notes(
        &self,
        group: &FrostGroup,
        ...
        let opened_nf = dkg_opened_nullifier(seats, note)?;
        // First add_ironwood_spend: THAT opened note → provider earned + DKG remainder.
        let pay = self.spend_note(group, seats, note, b"provider", earned_zat)?;
        if pay.nullifier != opened_nf {
            return Err(ZcashEscrowError::NoteOp(
                "DKG close: first add_ironwood_spend must consume THAT opened DKG note".into(),
            ));
        }
        // Remainder is the DKG-owned change after the earned spend (ZIP-317 may
        // have come from the note). Second add_ironwood_spend: THAT remainder
        // note → tenant. Do not re-shield a miner coinbase as if it were
        // FROST-owned, and do not clone the provider spend as a fake tenant refund.
        match pay.change.as_deref() {
            None => Err(ZcashEscrowError::NoteOp(
                "DKG close: remainder note missing after earned spend (not a wallet refund)".into(),
            )),
            Some(change) => {
                let refund =
                    self.spend_note(group, seats, change, b"tenant", change.amount_zat)?;
                if pay.spend_stand_in != IRONWOOD_SPEND_STAND_IN
                    || refund.spend_stand_in != IRONWOOD_SPEND_STAND_IN
                {
                    return Err(ZcashEscrowError::NoteOp(
                        "DKG close: ZIP-312 RedPallas FROST is not this module (keep frost-spend-v1 + ZIP-32 reconstructed seed)".into(),
                    ));
                }
                Ok((pay, refund))
            }
...
/// Ironwood SK is ZIP-32 (coin type 133) of the reconstructed DKG group seed.
/// Any valid threshold subset of the same DKG run yields the same SK. Never
/// derived from the public verifying key. The reconstructed FROST-ed25519
/// scalar is a *seed*, not orchard `SpendAuthorizingKey` (wrong curve). Spend
/// still requires frost-spend-v1 over the opened Ironwood nullifier.
...
fn ironwood_sk_from_seats(seats: &[FrostSeat]) -> Result<SpendingKey, ZcashEscrowError> {
    ...
    // frost-ed25519 SigningKey serialize is the 32-byte group scalar.
    // ZIP-32 seed must be that scalar — not a seat encoding (32..=252 hash blob).
    if secret.len() != 32 {
        return Err(ZcashEscrowError::NoteOp(
            "DKG reconstruct seed must be the 32-byte group scalar for ZIP-32 Ironwood SK".into(),
        ));
    }
    // ZIP-32 account 0 is the group receiver; try a few accounts if sk/ivk is ⊥.
    for i in 0u32..16 {
        let account = i
            .try_into()
            .map_err(|_| ZcashEscrowError::NoteOp("ZIP-32 account".into()))?;
        match SpendingKey::from_zip32_seed(&secret, 133, account) {
            Ok(sk) => return Ok(sk),
            Err(_) => continue,
        }
    }
    Err(ZcashEscrowError::NoteOp(
        "ZIP-32 could not derive Ironwood SpendingKey from reconstructed DKG group seed".into(),
    ))
}
...
fn open_group_ironwood_note(
    ...
    let note = decrypt_outputs(built.ironwood_bundle(), &fvk)
        .into_iter()
        .find(|n| {
            n.recipient() == addr
                && n.value().inner() == amount_zat
                && n.version() == NoteVersion::V3
        })
        .ok_or_else(|| {
            ZcashEscrowError::NoteOp(
                "DKG open: Ironwood V3 note not decryptable after build".into(),
            )
        })?;
    require_dkg_owns_opened(&fvk, &note)?;
...
fn spend_opened_ironwood(
    ...
    amount_zat: u64,
    frost_sig: &[u8],
) -> Result<(String, Option<IronwoodNote>, [u8; 32], bool), ZcashEscrowError> {
    let sk = ironwood_sk_from_seats(seats)?;
    let fvk = FullViewingKey::from(&sk);
    require_dkg_owns_opened(&fvk, opened)?;
    let change_addr = fvk.address_at(dest_diversifier(b"open"), Scope::External);
    ...
    // Spend authority is the DKG group: ZIP-32 Ironwood SK + frost-spend-v1
    // over this note's nullifier, then `add_ironwood_spend` of the opened note.
    // Prefer a shielded-only DKG Ironwood spend. Miner coinbase pays ZIP-317
    // only when the note cannot cover the fee — not wallet send-many, not a
    // coinbase re-shield as if FROST-owned.
    let (raw, built, shielded_only) = match build_spend_tx(
        esc,
        &fvk,
        &sk,
        opened,
        ...
        Ok(v) => (v.0, v.1, true),
        Err(e) if zip317_from_note_too_small(&e) => {
            let v = build_spend_tx(
                esc,
                &fvk,
                &sk,
                opened,
    ...
    require_frost_dest_memo(built.ironwood_bundle(), &fvk, dest, frost_sig)?;
    let change = if drain {
        None
    } else {
    ...
    let txid = esc.send_raw(&hex::encode(&raw))?;
    Ok((txid, change, nullifier, shielded_only))
}
...
/// ZIP-317 for a shielded-only DKG spend: one Ironwood spend plus dest, and
/// optionally a DKG change output. Probe amounts are 1 zat; fee is action-count.
fn zip317_shielded_fee(
    height: u64,
    cfg: &zcash_primitives::transaction::builder::BuildConfig,
    fvk: &FullViewingKey,
    opened: &IronwoodNote,
    ...
    let mut probe = TxBuilder::new(ZakuraParams, BlockHeight::from(height as u32), cfg.clone());
    probe
        .add_ironwood_spend::<()>(fvk.clone(), *opened, merkle_path)
        .map_err(|e| ZcashEscrowError::NoteOp(format!("add_ironwood_spend: {e:?}")))?;
    let one = Zatoshis::from_u64(1).map_err(|_| ZcashEscrowError::NoteOp("amount".into()))?;
    let ovk = fvk.to_ovk(Scope::External);
    probe
        .add_ironwood_output::<()>(Some(ovk.clone()), dest, one, memo.clone())
        .map_err(|e| ZcashEscrowError::NoteOp(format!("ironwood dest: {e:?}")))?;
    if extra_change {
        let change_addr = fvk.address_at(dest_diversifier(b"open"), Scope::External);
        probe
            .add_ironwood_output::<()>(Some(ovk), change_addr, one, memo.clone())
...
fn require_dkg_owns_opened(
    fvk: &FullViewingKey,
    opened: &IronwoodNote,
) -> Result<(), ZcashEscrowError> {
    let ivk = fvk.to_ivk(Scope::External);
    if ivk.diversifier_index(&opened.recipient()).is_none() {
        return Err(ZcashEscrowError::NoteOp(
            "DKG spend: opened Ironwood note is not owned by this DKG group (cannot spend miner transparent as if FROST-owned)".into(),
        ));
    }
    Ok(())
}

fn frost_spend_memo(frost_sig: &[u8]) -> Result<zcash_protocol::memo::MemoBytes, ZcashEscrowError> {
    if frost_sig.is_empty() {
        return Err(ZcashEscrowError::NoteOp(
            "DKG spend: frost-spend-v1 signature missing".into(),
        ));
    }
    let mut b = b"pir-dkg-frost-spend-v1:".to_vec();
    b.extend_from_slice(frost_sig);
    zcash_protocol::memo::MemoBytes::from_bytes(&b).map_err(|_| {
        ZcashEscrowError::NoteOp("DKG spend: frost-spend-v1 memo too long".into())
    })
}

fn require_frost_dest_memo(
    bundle: Option<&orchard::Bundle<orchard::bundle::Authorized, zcash_protocol::value::ZatBalance>>,
    fvk: &FullViewingKey,
    dest: orchard::Address,
    frost_sig: &[u8],
    ...
    let bundle = bundle.ok_or_else(|| {
        ZcashEscrowError::NoteOp("DKG spend: dest memo missing frost-spend-v1 (not a wallet send-many)".into())
    })?;
    let mut prefix = b"pir-dkg-frost-spend-v1:".to_vec();
    prefix.extend_from_slice(frost_sig);
    // OVK recovery proves this spend created dest; dest need not stay DKG-owned.
    let ovk = fvk.to_ovk(Scope::External);
    let found = bundle
        .recover_outputs_with_ovks(&[ovk])
        .into_iter()
        .any(|(_, _, n, _, memo)| n.recipient() == dest && memo.starts_with(&prefix));
    if !found {
        return Err(ZcashEscrowError::NoteOp(
            "DKG spend: dest memo missing frost-spend-v1 (not a wallet send-many)".into(),
        ));
    }
    Ok(())
}
...
fn mined_v6_tx(
    esc: &ZcashEscrow,
    txid: &str,
) -> Result<zcash_primitives::transaction::Transaction, ZcashEscrowError> {
    ...
    if hex_tx.is_empty() {
        return Err(ZcashEscrowError::NoteOp(format!(
            "DKG: mined tx {txid} missing hex (Zakura did not accept the Ironwood sendraw)"
        )));
    }
    parse_v6_tx(&hex_tx)
}

fn confirm_mined_dkg_open(
    esc: &ZcashEscrow,
    txid: &str,
    seats: &[FrostSeat],
    built: &IronwoodNote,
) -> Result<IronwoodNote, ZcashEscrowError> {
    let mined = mined_v6_tx(esc, txid)?;
    require_ironwood_only(&mined)?;
    let sk = ironwood_sk_from_seats(seats)?;
    let fvk = FullViewingKey::from(&sk);
    require_dkg_owns_opened(&fvk, built)?;
    let want_addr = built.recipient();
    let want_v = built.value().inner();
    let want_cmx = extracted_cmx(built);
    let found = decrypt_outputs(mined.ironwood_bundle(), &fvk)
        .into_iter()
        .find(|n| {
            n.recipient() == want_addr
                && n.value().inner() == want_v
                && n.version() == NoteVersion::V3
                && extracted_cmx(n) == want_cmx
        })
        .ok_or_else(|| {
            ZcashEscrowError::NoteOp(
                "DKG open: mined tx missing DKG Ironwood V3 note (not a wallet send-many)".into(),
            )
        })?;
    require_dkg_owns_opened(&fvk, &found)?;
    Ok(found)
}

fn confirm_mined_dkg_spend(
    esc: &ZcashEscrow,
    txid: &str,
    seats: &[FrostSeat],
    opened: &IronwoodNote,
    ...
    let sk = ironwood_sk_from_seats(seats)?;
    let fvk = FullViewingKey::from(&sk);
    let _nf = require_opened_nullifier(&mined, &fvk, opened)?;
    require_frost_dest_memo(mined.ironwood_bundle(), &fvk, dest, frost_sig)?;
    if shielded_only {
        if let Some(tb) = mined.transparent_bundle() {
            if !tb.vin.is_empty() {
                return Err(ZcashEscrowError::NoteOp(
                    "DKG spend: mined shielded-only tx has transparent inputs (cannot spend miner coinbase as if FROST-owned)".into(),
                ));
            }
        }
    } else {
...
fn build_spend_tx(
    ...
    match fee_src {
        SpendFee::FromNote => {
            // Shielded-only DKG Ironwood spend. Probe dest-only vs dest+change so
            // adding the DKG change output does not bump ZIP-317 and bounce the
            // spend onto miner coinbase as if that transparent were FROST-owned.
            let fee2 = zip317_shielded_fee(
                height,
                &cfg,
                fvk,
                opened,
            ...
            if drain {
                ...
                let dest_v = Zatoshis::from_u64(opened_v - fee2)
                    .map_err(|_| ZcashEscrowError::NoteOp("amount".into()))?;
                txb.add_ironwood_output::<()>(Some(ovk), dest, dest_v, memo)
                    .map_err(|e| ZcashEscrowError::NoteOp(format!("ironwood dest: {e:?}")))?;
            } else {
